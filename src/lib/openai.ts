...
export async function generateBookRecommendations(category: string, count: number = 5) {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "당신은 전문 독서 모임 큐레이터입니다. 독서 모임에 적합한 책을 추천해주세요."
      },
      {
        role: "user",
        content: prompt
...
export async function generateDiscussionQuestions(
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "당신은 전문 독서 토론 진행자입니다. 책의 핵심 주제와 의미를 깊이 있게 탐구할 수 있는 질문들을 제시해주세요."
      },
      {
        role: "user",
        content: prompt
...
export async function generateBookSummary(bookTitle: string, author: string) {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "당신은 전문 도서 큐레이터입니다. 책의 핵심 가치와 독서 모임에서의 활용 가치를 잘 설명해주세요."
      },
      {
        role: "user",
        content: prompt
